...
object EvergreenSearchService: SearchService {
    ...
    override suspend fun fetchCopyLocationCounts(recordId: Int, orgId: Int, orgLevel: Int): Result<List<CopyLocationCounts>> {
        return try {
            val response = GatewayClient.fetch(Api.SEARCH, Api.COPY_LOCATION_COUNTS, paramListOf(recordId, orgId, orgLevel), false)
    ...
    suspend fun fetchMulticlassQuery(queryString: String, limit: Int, shouldCache: Boolean): Result<OSRFObject> {
        return try {
            ...
            val response = GatewayClient.fetch(Api.SEARCH, Api.MULTICLASS_QUERY, params, shouldCache)
