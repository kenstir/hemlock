...
class CopyInformationViewAdapter(
    ...
    class ViewHolder(v: View, val groupCopiesBySystem: Boolean) : RecyclerView.ViewHolder(v) {
        ...
        fun bindView(clc: CopyLocationCounts) {
            val org = App.svc.consortium.findOrg(clc.orgId)

            if (groupCopiesBySystem) {
                majorLocationText.text = App.svc.consortium.findOrgNameSafe(org?.parent)
                ...
                majorLocationText.text = App.svc.consortium.findOrgNameSafe(clc.orgId)
